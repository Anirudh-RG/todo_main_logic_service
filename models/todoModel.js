...
// Update this function in your todoModel.js
async function getTodosWithRemindersForToday() {
  const today = new Date().toISOString().split('T')[0]; // This gives YYYY-MM-DD format
  console.log(`Looking for todos with deadline: ${today}`); // Add this log
  
  const result = await db.query(`
    SELECT t.*, u.email 
    FROM todos t 
    JOIN users u ON t.username = u.username 
    WHERE DATE(t.deadline_at) = DATE($1)
    AND u.email IS NOT NULL
  `, [today]);
  
  console.log(`Found ${result.rows.length} todos with reminders for today`); // Add this log
  return result.rows;
}
