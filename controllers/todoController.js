...
const { processReminders } = require('../services/reminderService'); // Add this import
...
// Update the addTodo function in your todoController.js
exports.addTodo = async (req, res) => {
  try {
    ...
    // Check for reminders when adding new todo - simplified date comparison
    if (date) {
      const today = new Date().toISOString().split('T')[0]; // YYYY-MM-DD
      const todoDate = new Date(date).toISOString().split('T')[0]; // YYYY-MM-DD
      
      console.log(`Today: ${today}, Todo date: ${todoDate}`); // Add this log
      
      if (todoDate === today) {
        console.log('New todo has today\'s deadline, checking reminders...');
        setTimeout(async () => {
          await processReminders();
        }, 1000); // Small delay to ensure todo is saved
      }
    }
